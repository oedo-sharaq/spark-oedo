...
object HRTDCUnpairedDecoder {
  ...
  val kHeaderTDCL: Int = 0x0b;   // tdc data
  ...
  def decode(binaryData: Array[Byte]): DecodedMeasurement = {
    ...
    for (i <- 0 until evtsize) {
      ...
      ih match {
        case `kHeaderTDCL` | `kHeaderTDCT` | `kHeaderTDCT1S` | `kHeaderTDCT1E` | `kHeaderTDCT2S` | `kHeaderTDCT2E` =>
          ...
          measurements :+= TDCData(ch, time, edge)
  ...
  class DecodeHRTDCUnpairedData extends UDF1[Array[Byte], DecodedMeasurement] {
  ...
  def registerUDF(spark: SparkSession): Unit = {
    spark.udf.register("decode_hrtdc_unpaired_segdata", new DecodeHRTDCUnpairedData(),
      StructType(Seq(
